...
import com.www.timesaleservice.aop.TimeSaleMetricsAspect;
  ...
  public MeterBinder redisMetrics(RedissonClient redissonClient) {
    return registry -> {
      Metrics.gauge("redis.memory_used_bytes", redissonClient, client -> {
        try {
          Map<String, String> info = client.getRedisNodes(RedisNodes.SINGLE).getInstance().info(
              RedisNode.InfoSection.MEMORY);
