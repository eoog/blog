...
public class AuthorizationDirective implements SchemaDirectiveWiring {
  ...
  public GraphQLFieldDefinition onField(
    ...
    String requiredRole = environment.getAppliedDirective("authorize").getArgument("role")
        .getValue();
    String requiredPermission = environment.getAppliedDirective("authorize")
        .getArgument("permission").getValue();
    ...
    DataFetcher<?> authDataFetcher = (DataFetchingEnvironment dataFetchingEnvironment) -> {
      if (!rolePermissionService.checkPermission(requiredRole, requiredPermission,
