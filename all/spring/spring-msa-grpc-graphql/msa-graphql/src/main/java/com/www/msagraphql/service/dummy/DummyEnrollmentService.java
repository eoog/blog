package com.www.msagraphql.service.dummy;
...
public class DummyEnrollmentService {
  ...
  private void initData() {
    ...
    payments.add(new Payment(paymentIdGenerator.incrementAndGet(), 100L, null, "COURSE", 100.00f,
        "Credit Card", LocalDateTime.now().minusDays(10).toString()));
    payments.add(
        new Payment(paymentIdGenerator.incrementAndGet(), 100L, null, "SUBSCRIPTION", 120.00f,
            "PayPal", LocalDateTime.now().minusDays(15).toString()));

    // Initialize some dummy enrollments
    enrollments.add(
        new Enrollment(enrollmentIdGenerator.incrementAndGet(), 100L, null, 100L, null, paymentId1,
            null, LocalDateTime.now().minusDays(5).toString()));
    enrollments.add(
        new Enrollment(enrollmentIdGenerator.incrementAndGet(), 100L, null, 200L, null, paymentId2,
            null, LocalDateTime.now().minusDays(3).toString()));

    // Initialize some dummy subscriptions
    subscriptions.add(
        new PlanSubscription(subscriptionIdGenerator.incrementAndGet(), 101L, null, paymentId3,
            null, LocalDateTime.now().minusDays(300).toString(),
            LocalDateTime.now().plusDays(65).toString(), "Expired"));
    subscriptions.add(
        new PlanSubscription(subscriptionIdGenerator.incrementAndGet(), 102L, null, paymentId4,
            null, LocalDateTime.now().minusDays(200).toString(),
            LocalDateTime.now().plusDays(160).toString(), "Expired"));
  }
  ...
  public Payment purchaseCourse(long userId, long courseId, double amount, String paymentMethod) {
    Payment newPayment = new Payment(paymentIdGenerator.incrementAndGet(), userId, null, "COURSE",
        (float) amount, paymentMethod, LocalDateTime.now().toString());
    payments.add(newPayment);
    return newPayment;
  }

  public Payment purchaseSubscription(long userId, double amount, String paymentMethod) {
    Payment newPayment = new Payment(paymentIdGenerator.incrementAndGet(), userId, null,
        "SUBSCRIPTION", (float) amount, paymentMethod, LocalDateTime.now().toString());
    payments.add(newPayment);
    return newPayment;
  }
  ...
  public boolean checkSubscriptionAccess(long userId) {
    ...
    return subscriptions.stream().anyMatch(
        s -> s.getUserId() == userId && (now.isAfter(LocalDateTime.parse(s.getStartDate()))
            && now.isBefore(LocalDateTime.parse(s.getEndDate()))));
  }
