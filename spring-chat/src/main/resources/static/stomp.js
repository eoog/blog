...
stompClient.onConnect = (frame) => {
  ...
  // 새로운 메시지 발행한거
    stompClient.subscribe('/sub/chats/news' ,
        (chatMessage) => {
            toggleNewMessageIcon(JSON.parse(chatMessage.body) , true); // 신규 메시지 발행된 채팅방 아이디
        }
        )
...
function toggleNewMessageIcon(chatroomId, toggle) {
  console.log("토클 =" ,chatroomId , toggle)
    if (toggle) {
...
function enterChatroom(chatroomId, newMember) {
  ...
  toggleNewMessageIcon(chatroomId , false); // 신규메시지 닫기
